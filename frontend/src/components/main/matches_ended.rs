...
pub struct MatchesEnded {
    ...
    games: Vec<Game>,
    ended_matches: Vec<Match>,

    matches: Vec<Match>,
}

impl Component for MatchesEnded {
    type Message = Msg;
    type Properties = ();
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            ...
            Msg::MatchesStore(state) => {
                let state = state.borrow();
                self.ended_matches = state.matches_ended.clone();
        ...
        html! {
            <ul class="flex flex-col overflow-auto">
            {
                if self.is_loading {
                    ...
                    html! { <div class="bg-blue rounded-md p-1 text-center text-white" >{ "No ended matches to show" }</div> }
                } else {
                    self.games.clone().into_iter().map(|game| {
                        ...
                        html! {
                            <li key={ game.id }>
                                <GameMatches
                                    event_type={GameEventType::Ended}
                                    id={game.id}
                                    name={game.name}
